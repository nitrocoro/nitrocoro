...
Http2ResponseSink::Http2ResponseSink(std::weak_ptr<Http2Session> session, uint32_t streamId)
    : session_(std::move(session)), streamId_(streamId)
{
}

Task<> Http2ResponseSink::send(const http::HttpResponse & resp, std::string_view body,
                               bool ignoreBody)
{
